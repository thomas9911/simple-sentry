...
fn parse_datetime_without_timezone<'de, D>(deserializer: D) -> Result<OffsetDateTime, D::Error> where D: Deserializer<'de> {
    let s = String::deserialize(deserializer)?;

    let format = format_description!(version = 2, "[year]-[month]-[day]T[hour]:[minute]:[second].[subsecond]");
